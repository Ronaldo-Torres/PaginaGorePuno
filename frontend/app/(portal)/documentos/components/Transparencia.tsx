...
export default function transparenciaSection({ }: transparenciaSectionProps) {

  // Estados para transparencia
  const [transparencia, settransparencia] = useState<any[]>([]);
  ...
  const tipoId = 6; // Id de TIPO_DOC = TRANSPARENCIA
  ...
  const getTiposDocumentos = async () => {
    try {
      const response = await PrincipalService.getTiposDocumentos("D");
  ...
  // Función para obtener transparencia
  const gettransparencia = async () => {
    ...
    try {
      ...
      const tipos = await PrincipalService.getTiposDocumentos("D");
      ...
      // console.log("tipoId: ", tipoId, "enviarAnio: ", enviarAnio, "currentPage: ", currentPage, "itemsPerPage: ", itemsPerPage)

      const response = await DocumentoService.getDocumentosByTipo(
        // tipoId, // Id de TIPO_DOC = TRANSPARENCIA
        tipoDoc.id,
      ...
      settransparencia(response.content || []);
  ...
  // Efecto para cargar transparencia
  useEffect(() => {
      ...
      >
        <div className="p-4 border-b border-gray-100 bg-[#184482]/5 flex justify-between items-center">
          <h2 className="flex items-center gap-2 font-medium text-[#184482]">
            ...
            <span className="text-[#184482] font-semibold">Transparencia</span>
            {totalItems > 0 && (
        ...
        <div className="p-6">
          {/* Filtros específicos para transparencia */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Input
                    type="text"
                    placeholder="Buscar transparencia..."
                    value={searchTerm}
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              <div className="text-center py-8 text-gray-500">
                ...
                <p className="text-sm">
                  No se encontraron transparencia
                </p>
              </div>
            )}
          </div>
