...
import DocumentoModalSGD from "../documento-modalSGD";

import UltimosCinco from "./UltimosCincoDocumentos"

interface DocumentosOficialesSectionProps { }

export default function DocumentosOficialesSection({ }: DocumentosOficialesSectionProps) {
  ...
  const [isModalSGDOpen, setIsModalSGDOpen] = useState(false);
  ...
  const getTiposDocumentos = async () => {
    try {
      const response = await PrincipalService.getTiposDocumentos("A");
  ...
  const getDocumentos = async () => {
    ...
    try {
      ...
      console.log("tipoDoc: ", tipoDoc.id)
      ...
      const response = await PrincipalService.getDocumentosByTipo(
        ...
        activeSearchTerm,
        anioSeleccionado,
        null, // No filtrar por consejero en la vista general
        "A"
      )
  ...
  // Función para manejar la vista de documentos (según año)
  const handleVerDocumento = (documento: any) => {
    ...
    if (anioDocumento >= 2025) {
      setIsModalSGDOpen(true);
    } else {
      setIsModalOpen(true);
    }
  };
      ...
      >
        ...
        <div className="p-4 border-b border-gray-100 bg-[#184482]/5 flex justify-between items-center">
          ...
          <h2 className="flex items-center gap-2 font-medium text-[#184482]">
            <FaFileAlt className="h-4 w-4 text-[#184482] font-semibold" />
            <span className="text-[#184482] font-semibold">Documentos Oficiales</span>
            {totalItems > 0 && (
        ...
        <UltimosCinco
          tipoId={2} // codigo de Documentos Oficiales
          loading={loading}
