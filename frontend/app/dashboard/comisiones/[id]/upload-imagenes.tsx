...
import { fetchImages, uploadImages, deleteImage } from "./actions";
...
export function PhotoGalleryUpload({
  ...
  const loadExistingImages = async () => {
    setIsLoading(true);
    try {
      const images = await ComisionService.fetchImages(data.id);
      ...
      Array.from(e.target.files).forEach((file) => {
        ...
        if (file.size > 50 * 1024 * 1024) {
          toast({
            ...
            description: `${file.name} excede el tamaño máximo de 50MB.`,
          });
          return;
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      newImages.forEach((image, index) => {
        ...
        formData.append(`actividadId`, data.id as string);
      });

      console.log(formData);
      const result = await ComisionService.createImagen(formData);
  ...
  const handleDeleteImage = async () => {
    ...
    try {
      const result = await ComisionService.deleteImagen(imageToDelete.id);
      try {
        await loadExistingImages();
    ...
    >
      <DialogContent className="sm:max-w-[900px] w-[90vw]">
        <DialogHeader>
          ...
          <DialogDescription>
            Ver fotos existentes y subir nuevas imágenes.
          </DialogDescription>
        </DialogHeader>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                ...
                                permanentemente la imagen de nuestros
                                servidores.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
