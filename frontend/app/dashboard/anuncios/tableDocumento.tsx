...
export function TableDocumento() {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await AnuncioService.deleteAnuncio(dataToDelete.id);
  ...
  const onUpdateStatus = async (id: number, status: boolean) => {
    try {
      await AnuncioService.activarAnuncio(id, status);
      fetchData();
    } catch (error) {
    ...
    <div className="mx-4 sm:mx-6 md:mx-10 py-6 transition-colors duration-200 ease-in-out">
      <div className="space-y-4">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <div className="text-xl sm:text-2xl font-bold">Anuncios</div>
