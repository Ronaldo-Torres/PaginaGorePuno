...
export function PhotoGalleryUpload({
  ...
  const loadExistingImages = async () => {
    setIsLoading(true);
    try {
      const images = await BoletinService.fetchImages(data.id);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      newImages.forEach((image, index) => {
        ...
        formData.append(`boletinId`, data.id as string);
      });

      console.log(JSON.stringify(formData));
      const result = await BoletinService.createImagen(formData);
      ...
      try {
        ...
        throw new Error((error as string) || "Error al subir las imágenes");
      }
    } catch (error) {
      toast({
        variant: "destructive",
  ...
  const handleDeleteImage = async () => {
    ...
    try {
      setIsDeleting(true);
      await BoletinService.deleteImagen(imageToDelete.id);
  ...
  const openDeleteDialog = (image: ImageData, e: React.MouseEvent) => {
    e.stopPropagation(); // Evita que se active el handlePrincipal
  ...
  const handlePrincipal = async (image: ImageData) => {
    try {
      ...
      await BoletinService.updateImagen(image.id, {
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-[900px] w-[90vw]">
        <DialogHeader>
          <DialogTitle>Imágenes relacionadas</DialogTitle>
          <DialogDescription>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {isLoading
                  ...
                  : existingImages.map((image) => (
                      ...
                      >
                        {image.esPrincipal && (
                          <div className="absolute top-0 left-0 bg-teal-500 text-white text-xs p-1">
                            Principal
                          </div>
                        )}
                        <Image
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {newImages.map((image) => (
                  ...
                  >
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-2 bg-opacity-50 text-white text-xs truncate">
                      {image.file.name}
                    </div>
                  </div>
                ))}
