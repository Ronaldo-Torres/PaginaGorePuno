...
import Agregar from "./agregar-link";
...
export function TableLink() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await PortadaService.getLinks(
        currentPage,
        rowsPerPage,
        searchQuery,
        tipoDocumento
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await PortadaService.deleteLink(dataToDelete.id);
      setLinks(links.filter((d) => d.id !== dataToDelete.id));
      fetchData();
  ...
  const onUpdateStatus = async (id: number, status: boolean) => {
    try {
      await PortadaService.activarLink(id, status);
      fetchData();
    } catch (error) {
    ...
    <div className="mx-4 sm:mx-6 md:mx-10 py-6 transition-colors duration-200 ease-in-out">
      <div className="space-y-4">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <div className="text-xl sm:text-2xl font-bold">Link de interes</div>
            <p className="text-sm sm:text-base text-muted-foreground">
              Registro de link de interes
            </p>
          </div>
